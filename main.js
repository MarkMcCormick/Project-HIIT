...
function populateWorkoutDropdown() {
  ...
  fetch('http://localhost:8080/get-all-activities')
    ...
    .then(activities => {
      ...
      activities.forEach(activity => {
        ...
        option.value = activity.activity_type;  // Set its value to the activity ID
        option.text = activity.activity_name; // Set its text content to the activity name

        // Append this option to the select dropdown
        console.log(option);
...
function hideAllSets() {
  // Hide all sets for all activities
  for (var activity of addedActivities) {
      var sets = document.querySelectorAll('.set.' + activity);
...
// Function to show sets for a specific activity
function showSetsForActivity(activity) {
  var sets = workoutSets[activity];
  if (sets) {
...
function addNewActivity() {
  var selectedOption = document.getElementById("activityName").selectedOptions[0];
  var selectedActivity = selectedOption.textContent;

  if (addedActivities.includes(selectedActivity)) {
...
function checkActivityType() {
  ...
  var repsElement = document.querySelector('#reps_input');
  var weightElement = document.querySelector('#weight_input');

  var reps = repsElement ? repsElement.value : null;
  var weight = weightElement ? weightElement.value : null;

  window.repsValue = reps;
  window.weightValue = weight;
  
  var workoutDropdown = document.getElementById("activityName");
  var selectedWorkout = workoutDropdown.value;

  var workoutName = selectedWorkout.workoutName;
...
workoutDropdown.addEventListener('change', function() {
  ...
  var selectedWorkoutSets = document.getElementsByClassName('set ' + workoutDropdown.options[workoutDropdown.selectedIndex].dataset.workoutName);
...
workoutDropdown.addEventListener('change', function() {
  ...
  var selectedActivity = workoutDropdown.options[workoutDropdown.selectedIndex].dataset.workoutName;
...
function newSet(){
  ...
  newSetContainer.className = 'set ' + workoutDropdown.value; // Add the 'set' class and a class for the selected workout
...
removeButton.addEventListener('click', function() {
  ...
  workoutActivities[selectedActivity] = workoutActivities[selectedActivity] || [];
  workoutActivities[selectedActivity].push(newSetContainer);
...
var activityId = document.getElementById('activityName').value;

var setsElements = document.querySelectorAll('.set');
//var repsElements = document.querySelectorAll('.set input[type="number"][name="reps"]');
//var weightsElements = document.querySelectorAll('.set input[type="number"][name="weights"]');

var repsElements = document.querySelectorAll('#reps');
var weightsElements = document.querySelectorAll('#weights');


var sets = Array.from(setsElements).map(element => element.value);

console.log("Set elements:", setsElements);
var reps = Array.from(repsElements).map(element => element.value);
var weights = Array.from(weightsElements).map(element => element.value);


  // Create an array to store the data of each set
  var workoutData = [];

  // Iterate over each set to collect the data
  for (var i = 0; i < sets.length; i++) {
    var set = sets[i];
    var rep = reps[i];
    var weight = weights[i];

    // Create an object representing the data of the current set
    var setData = {
      set: set,
      rep: rep,
      weight: weight
    ...
    // Push the set data object to the array
  ...
  // Make a POST request to the server to add the workout data to the user_progress table
  fetch('/addActivity', {
    method: 'POST',
    headers: {
    ...
    body: JSON.stringify({
      activityId: activityId, // Include the selected activity id
